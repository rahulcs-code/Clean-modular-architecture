...
const _routePageChecker = TypeChecker.fromUrl(
  'package:clean_modular_architecture/src/generators/annotations.dart#RoutePage',
);
const _routeGuardChecker = TypeChecker.fromUrl(
  'package:clean_modular_architecture/src/generators/annotations.dart#RouteGuard',
);
...
class RouteGenerator extends Generator {
  ...
  FutureOr<String?> generate(LibraryReader library, BuildStep buildStep) async {
    ...
    for (final annotatedElement in library.annotatedWith(_routePageChecker)) {
    ...
    for (final annotatedElement in library.annotatedWith(_routeGuardChecker)) {
      final element = annotatedElement.element;
      if (element is ClassElement) {
  ...
  String _generateRoutes(List<_RouteInfo> routes, List<_GuardInfo> guards) {
    ...
    for (final route in routes) {
      final className = route.element.name ?? 'UnknownPage';
      final path = route.annotation.peek('path')?.stringValue ?? '/';
